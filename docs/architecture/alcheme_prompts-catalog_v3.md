...
| **Status** | Phase 1 Complete (Phase 2 で 10 エージェント構成に拡張済み) |
| **Related** | alcheme_PRD_v4.md, alcheme_design-doc_v1.md, CLAUDE_CODE_INSTRUCTIONS_v1_3.md |

> 本ドキュメントは Phase 1 MVP の全エージェント（4体: Concierge, Inventory Manager, Product Search, Cosmetic Alchemist）のシステムプロンプト、入出力 JSON スキーマ、期待される振る舞い、エッジケース対応を定義する。Phase 2 で追加された 6 エージェント（Simulator, Memory Keeper, Trend Hunter, TPO Tactician, Profiler, Makeup Instructor）のプロンプト定義は `agent/alcheme/` 配下の各 `.py` ファイルを正とする。
...
| 3.1 | 2026-02-17 | Phase 2 完了ステータス更新。本ドキュメントは Phase 1 の 4 エージェント定義を収録。Phase 2 で追加された 6 エージェント（Simulator, Memory Keeper, Trend Hunter, TPO Tactician, Profiler, Makeup Instructor）のプロンプト定義は `agent/alcheme/` 配下の各 `.py` ファイルを正とする。 |
