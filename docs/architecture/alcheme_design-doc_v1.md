...
│  │  │  │  Phase 4: 体験出力                         │  │     │   │
│  │  │  │  ┌────────────┐                            │  │     │   │
│  │  │  │  │ Simulator  │ (Gemini 2.0 Flash Image)  │  │     │   │
│  │  │  │  └────────────┘                            │  │     │   │
│  │  │  └───────────────────────────────────────────┘  │     │   │
│  │  └─────────────────────────────────────────────────┘     │   │
│  └──────────────────────────────────────────────────────────┘   │
...
| **Gemini 2.0 Flash Image** | 仕上がりプレビュー・AIキャラクターイメージ生成。$0.039/image のコスト効率。GA済み。フォールバックとして Gemini 2.0 Flash（ネイティブ画像生成対応）も利用可能。 |
...
| **天気API（二段構え）** | TPO Tactician の天気取得。① **Google Weather API** (`weather.googleapis.com`) を第一候補として試行 ② **Open-Meteo API** (`api.open-meteo.com`) をフォールバックとして使用。Google Weather API は日本を地理的にサポートしていないため（HTTP 404）、日本国内では実質 Open-Meteo が使用される。`GOOGLE_WEATHER_API_KEY` 環境変数が必要（Google側）。Open-Meteo は API キー不要、**CC BY 4.0 ライセンス**（クレジット表記が必要、設定画面に表示済み）、無料枠 10,000 コール/日。 | Phase 2〜 ✅ BUG-001 解決済み |
...
│
├── suggestions/                    # 買い足し候補（Phase 2〜）
│   └── {userId}/
│       └── items/
│           └── {itemId}            # AI提案 or 手動追加の買い足し候補
│
├── conversations/                  # チャット履歴（Phase 2〜）
│   └── {userId}/
│       └── items/
│           └── {conversationId}    # 会話メタデータ（タイトル、作成日時）
│
...
└── root_agent: Concierge Bot (LlmAgent - Gemini 2.5 Flash)
    ...
    │   │
    │   ├── simulator (LlmAgent - Gemini 2.0 Flash Image)
    │   │     レシピの仕上がりプレビュー画像を生成。
...
| **仕上がりプレビュー** (レシピ→画像生成) | < 10秒 | Gemini 2.0 Flash Image。非同期生成 + プレースホルダーUI。 |
...
**決定:** 仕上がりプレビューには Gemini 2.0 Flash Image を使用。ユーザーの顔写真ではなく、AIキャラクターイメージにメイクを施す方式とする。
