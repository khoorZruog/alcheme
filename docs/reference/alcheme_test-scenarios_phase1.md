...
| **Status** | Phase 1 Complete (Phase 2 テストは manual_e2e_test_guide.md を参照) |
...
| 1.1 | 2026-02-17 | Phase 2 完了に伴いステータス更新。Phase 2 機能（Beauty Log, SNS, Suggestions, Chat History 等）の手動テストは `docs/guides/manual_e2e_test_guide.md` v2.0 を参照。テスト実績（Vitest 127+ / Playwright 12 / pytest 100+）を反映。 |
...
## Phase 2 テストカバレッジ（参照先）

Phase 2 (Batch 0.5〜8.5) で追加された以下の機能のテストシナリオは、各専用ドキュメントを参照すること。

| 機能 | 自動テスト | 手動テスト |
|------|----------|----------|
| Beauty Log（メイク日記） | Vitest (hooks/components) | `manual_e2e_test_guide.md` MT-09 |
| SNS（フィード・フォロー・いいね・コメント） | Vitest (hooks/components) | `manual_e2e_test_guide.md` MT-10 |
| 買い足し候補（Suggestions） | Vitest (hooks/components) | `manual_e2e_test_guide.md` MT-08 |
| チャット履歴管理 | Vitest (hooks) | `manual_e2e_test_guide.md` MT-06 |
| 仕上がりプレビュー（Simulator） | pytest (agent tests) | `manual_e2e_test_guide.md` MT-05 |
| 登録4パターン（スキャン/楽天/Web/手動） | Vitest (components) | `manual_e2e_test_guide.md` MT-03 |
| ショッピング相談（相性・比較） | pytest (agent tests) | `manual_e2e_test_guide.md` MT-07 |
| プロフィール・設定 | Vitest (components) | `manual_e2e_test_guide.md` MT-11 |

### テスト実績サマリー (2026-02-17 時点)

| テスト層 | 件数 | ステータス |
|---------|------|----------|
| Python Unit (pytest) | 100+ | ✅ 全パス |
| Frontend Unit (Vitest) | 127+ | ✅ 全パス |
| E2E (Playwright) | 12 | ✅ 全パス |
| 手動テスト (manual_e2e_test_guide) | 233 | 📋 テスト対象 |

### 既知のバグ（テスト時の注意）

| # | バグ | 影響するテスト |
|---|------|-------------|
| BUG-001 | 天気API 取得失敗 | AGT-14 (TPO Tactician 連携)、MT-05 (チャット天気コンテキスト) |
| BUG-002 | 買い足し手動追加失敗 | MT-08 (買い足し候補 手動追加テスト) |
