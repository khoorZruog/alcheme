...
* **可視化:** **Gemini 2.0 Flash** が「そのメイクをした時の仕上がりイメージ」をAIキャラクターで生成。直感的に選べます。
...
レシピ生成後、サーバーサイドで **Gemini 2.0 Flash** を使ってAIキャラクターにメイクを施したプレビュー画像を自動生成。15秒のタイムアウト付きで非同期処理し、`preview_image` SSEイベントで配信します。ユーザーの顔写真は一切使用せず、プライバシーを保護しています。
