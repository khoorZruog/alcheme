...
export default function FollowingPage() {
  ...
  const { user } = useAuth();
  const { users, isLoading, mutate } = useFollowList(userId, "following");
  ...
  return (
    <div>
      <PageHeader title="フォロー中" backHref={`/profile/${userId}`} />
      ...
      <div className="px-4 py-2">
        ...
        {!isLoading && users.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-text-muted">フォロー中のユーザーはいません</p>
          </div>
        )}
